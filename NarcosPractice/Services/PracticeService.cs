...
public class PracticeService
{
    ...
    public void GuideTo(CCSPlayerController player, Lineup lineup)
    {
        ...
        // of just aiming the camera. Only yaw goes here - the aim reference
        // marker below shows the exact saved pitch+yaw to look at instead.
        //
        // There is no safe way for the plugin to force a player's view angle:
        // writing m_angEyeAngles directly via schema crashed the server (not a
        // networked field), and running "setang" through ExecuteClientCommand
        // is flatly rejected by the engine ("missing required FCVAR flag") -
        // that command only accepts genuine client-typed input, not
        // server-injected commands, as a deliberate anti-cheat protection.
        // The aim-reference dot the player aims at themselves is the only
        // viable option here.
    ...
    private const float LineupAimEnterAngleDegrees = 3f;
    private const float LineupAimExitAngleDegrees = 6f;
